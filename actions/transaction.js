...
export async function createTransaction(data) {
    try {
        ...
        const decision = await aj.protect(req, {
            userId,
            requested: 1, //No of tokens to consume
        });
...
export async function voiceAssistant(text) {
    try {
        const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });
        const prompt = `
            Analyze this voice input and extract the following information in JSON format:
            - Total amount (just the number)
            - Date (in ISO format)
            - Description or items purchased (brief summary)
            - Merchant/store name
            ...
            If it's not a valid transaction, return an empty object`;
        const result = await model.generateContent([
            {
                inlineData: {
                    data: text,
